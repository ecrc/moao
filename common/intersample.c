...
void 
intersample_prepare(struct isample_struct *isample, long nidx, long np, float Dtel, char* path){
  /*
   > fourier.N
   512
   > fourier.ud
...
fits_save_double_2d_square(long size, double* buf, char* file_name){
  ...
  if ( fits_write_img(fptr, TDOUBLE, fpixel, nelements, buf, &status) )
    fits_report_error(stderr, status);
...
intersample_save(struct isample_struct *isample, char* file_name){
  ...
  int bitpix   =  DOUBLE_IMG;
  long naxis    =   2;  /* 2-dimensional image                            */
  long naxes[2] = { isample->N, isample->N };
  long fpixel, nelements;
  ...
  if ( fits_write_img(fptr, TDOUBLE, fpixel, nelements, isample->dphi, &status) )
    fits_report_error(stderr, status);
  
  if ( fits_close_file(fptr, &status) )
    fits_report_error(stderr, status);
  //fits_save_double_2d_square(isample->N, isample->dphi, file_name);
}
