...
int main(int argc, char* argv[])
{
	if(argc < 4)
	{
		printf("USAGE: %s <nswsp-start> <nssp-end> <nssp-step>\n", argv[0]);
		exit(1);
	}
	...
	printf("                    Cmm generation                        "); 
  	printf("                    Cpm generation                        "); 
  	printf("                    Cpp generation                        "); 
  	printf("\n");
  	printf("-------------------------------------------------------   "); 
  	printf("-------------------------------------------------------   "); 
  	printf("-------------------------------------------------------   "); 
  	printf("\n");
  	printf("M         N     cpu (ms)     gpu (ms)     err             ");
  	printf("M         N     cpu (ms)     gpu (ms)     err             ");
  	printf("M         N     cpu (ms)     gpu (ms)     err             ");
  	printf("\n");
  	printf("-----   -----   ----------   ----------   -------------   ");
	...
	for(int ni = nssp_start; ni <= nssp_end; ni += nssp_step)
	{
		write_sys_file(ni);
		...
		init_tomo_sys(&tomo);
  		...
  		nsubaps_offaxis = 0;
  		for (int i = 0; i < tomo.Nw-1; i++) nsubaps_offaxis += tomo.Nsubap[i];
		
  		int M, N, m, p, size_cpu, size_gpu;
  		int LDA, LDA_;
  		...
  		// Cmm generation
  		{
			...
			printf("%-10.2f   ", cpu_time);
  			...
  			double *da;
  			e = cudaMalloc((void**)&da, size_gpu * sizeof(double));
  		...
  		//if(type == 3)
  		{
			...
			double *db;
  			e = cudaMalloc((void**)&db, size_gpu * sizeof(double));
		...
		//if(type == 4)
		{
			...
			LDA_ = ((M+31)/32)*32;
  			...
  			double *dc;
  			e = cudaMalloc((void**)&dc, size_gpu * sizeof(double));
